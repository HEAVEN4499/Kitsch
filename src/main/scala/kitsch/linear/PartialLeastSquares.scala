//package kitsch.linear
//
//import breeze.linalg._
//import breeze.linalg.eigSym.EigSym
//import breeze.numerics._
//import kitsch.Kitsch
//import kitsch.rdd.{TextRDD, RDD}
//
//import scala.reflect.ClassTag
//
//case class StandardizedModel[ModelT](model: ModelT,
...
//}
//
//object PartialLeastSquares {
//  def train(rdd: RDD[String],
//            split: String => Seq[String],
//            A: Int,
//            responseColumnIndices: Seq[Int] = Array(0)): RDD[Double] = {
//    val responseColumnIndexToColumnInResponseMatrixMap =
//      responseColumnIndices.zipWithIndex
//        .foldLeft(Map.empty[Int, Int]) { (m, pair) => m + (pair._1 -> pair._2) }
//    val lineCount = rdd.cache.map(split).count
//    val rows = rdd.cache.map(split).iterator
//    val (x, y): (DenseMatrix[Double], DenseMatrix[Double]) = if (lineCount > 0) {
//      var fields = rdd.cache.map(split).collect.head.map(_.trim.toDouble)
//      val columnCount = fields.length
//      val M: Int = responseColumnIndices.length // number of response variables
//      val N: Int = lineCount.toInt // number of rows
//      val K: Int = columnCount - M // number of predictor variables
//
...
//    rdd.kitsch.parallelize(DayalMcGregor.VIP.computeVIP(model, x, y).toArray)
//  }
//}
