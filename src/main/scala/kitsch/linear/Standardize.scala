...
import kitsch.util._
import breeze.linalg.DenseMatrix

object Standardize {
  case class StandardizationFactors(meanOfColumns: DenseMatrix[Double],     // meanOfColumns is a row vector
  ...
  // M is a (R x C) matrix
  // meanOfColumnsInM is a (1 x C) matrix
  def scaleColumnsWithStdDevs(M: DenseMatrix[Double],
                               stdDevOfColumnsInM: DenseMatrix[Double]): DenseMatrix[Double] =
    M :/ Utils.vertcat(stdDevOfColumnsInM, M.rows)
